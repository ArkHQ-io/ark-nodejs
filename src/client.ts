...
export class Ark {
   ...
   */
  constructor({
    baseURL = readEnv('ARK_BASE_URL'),
    apiKey = readEnv('ARK_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ArkError(
        "The ARK_API_KEY environment variable is missing or empty; either provide it, or instantiate the Ark client with an apiKey option, like new Ark({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.arkhq.io/v1`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Ark.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ARK_LOG'), "process.env['ARK_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.arkhq.io/v1';
  }
  ...
  static ArkError = Errors.ArkError;
